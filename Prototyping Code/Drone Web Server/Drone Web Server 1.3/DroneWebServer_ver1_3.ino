...
.card{border:1px solid #ddd;border-radius:12px;padding:12px;box-shadow:0 1px 4px rgba(0,0,0,.05);max-width:760px;margin:0 auto}
#cnv{width:100%;height:280px;display:block}
...
#stream{display:block;width:100%;max-width:480px;aspect-ratio:3/2;border-radius:8px;border:1px solid #ddd;margin:10px auto 0 auto}
.caption{font-size:12px;color:#555;text-align:center;margin-top:4px}
</style>
</head><body>
  <div class="card">
    <h2 style="margin-top:0;">ESP32-CAM – Clock Graph + HVGA Video</h2>

    <div>State: <span id="state" class="pill">—</span></div>
    <div class="mono" style="margin-top:4px">Raw: <span id="raw">—</span> | Baseline: <span id="base">—</span> | Threshold: <span id="th">—</span></div>

    <div style="margin-top:8px;display:flex;gap:12px;align-items:center;">
      <label>Trigger % <span id="pval">8</span>%</label>
      <input id="pct" type="range" min="1" max="30" value="8">
    </div>

    <canvas id="cnv" width="760" height="280"></canvas>

    <!-- VIDEO directly below graph -->
    <img id="stream" alt="HVGA MJPEG stream">
    <div class="caption mono">Streaming MJPEG @ HVGA (480×320)</div>
  </div>
...
void setup() {
  ...
  // camera_config_t config;  (keep your existing fields)
  config.pixel_format = PIXFORMAT_JPEG;
  config.frame_size   = FRAMESIZE_HVGA;   // 480x320
  config.jpeg_quality = 24;               // higher number = lower quality (smaller frames)
